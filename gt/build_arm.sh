...
BUILD_ESSENTIAL="apt-get -y update && \
	apt-get -y install build-essential bison zlib1g-dev libtool cmake gcc g++ libc6 gawk"
...
if [ ! -f ${PWD_PATH}/succeed/binutils ]; then
    ...
    " ../configure --enable-gold --prefix=/opt/shared/sok-arm/gt/build_arm/executable_binutils_debug --disable-werror --enable-shared --enable-ld=default --enable-plugins --enable-64-bit-bfd --with-system-zlib --enable-multilib --with-arch=armv7-a --with-fpu=vfpv3-d16 --with-float=hard"
...
if [ ! -f ${PWD_PATH}/succeed/gcc ]; then
	...
	--disable-checking   \
        --program-suffix=-8.1 \
	--disable-lto \
        --enable-languages=c,c++,fortran && make -j12"
    ...
    cp -r "${SRC_PATH}/gcc/gcc-8.1.0/gcc" ${GCC_PATH}/gcc
      ...
      # gcc
    eval "cd $GCC_BUILD_PATH && make -j12 &&   make install"
...
if [ ! -f ${PWD_PATH}/succeed/clang ]; then
    ...
    cd build_clang
    cmake ${LLVM_PATH} -DCMAKE_EXE_LINKER_FLAGS_DEBUG="-I/usr/local/include -L/usr/local/lib -lprotobuf -lpthread" -DLLVM_ENABLE_RTTI=ON -DLLVM_BINUTILS_INCDIR=$BINUTILS_PATH/include -DCMAKE_BUILD_TYPE=Release -DLLVM_TARGETS_TO_BUILD="ARM"
